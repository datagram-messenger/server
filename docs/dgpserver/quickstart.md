...
func serve(ctx context.Context, ln net.Listener, serverKey dgproto.StaticKey, clientKey [32]byte) error {
    ...
    if err := commands.Handle(1, dgpserver.HandlerFunc(
        func(ctx *dgpserver.Context, payload any) error {
            request := payload.(*dgproto.EncryptedData)
...
1. Load a persistent 32-byte server Noise static private key with `dgproto.LoadStaticKey`. Do not generate a new identity on every restart.
...
4. Set finite handshake/write/liveness limits and bounded queue/connection capacities in `dgproto.ServerConfig`.
