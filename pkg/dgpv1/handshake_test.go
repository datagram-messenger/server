...
func TestHandshakeDeterministicVector(t *testing.T) {
	...
	wantSession := [16]byte{0xa1, 0x28, 0x52, 0x30, 0x70, 0x52, 0x26, 0x76, 0x02, 0x2d, 0xd4, 0x23, 0x48, 0xb2, 0xc2, 0xff}
	wantSend := [KeySize]byte{0xf0, 0x1a, 0x7e, 0xb6, 0x94, 0x13, 0xb2, 0x3a, 0x6c, 0x38, 0x7b, 0x4e, 0xc5, 0x9e, 0xc2, 0xdb, 0x33, 0xf3, 0xe2, 0x8c, 0x67, 0xc0, 0x60, 0x7d, 0x01, 0x70, 0xee, 0xb9, 0xef, 0x37, 0x60, 0x83}
	wantReceive := [KeySize]byte{0x44, 0xcb, 0x76, 0x8d, 0xb5, 0xa1, 0x93, 0xc3, 0xc9, 0xbe, 0x06, 0xd9, 0x66, 0x5e, 0x0a, 0x73, 0x8a, 0x1d, 0xee, 0x54, 0x82, 0x67, 0x11, 0x25, 0xa0, 0x89, 0x1d, 0xad, 0x52, 0x62, 0x4e, 0xa8}
	if result.SessionID != wantSession || result.SendKey != wantSend || result.ReceiveKey != wantReceive {
		t.Fatalf("deterministic traffic keys changed: session=%x send=%x receive=%x", result.SessionID, result.SendKey, result.ReceiveKey)
	}
}
